#' Combines SIMPLS and Sparse SIMPLS algorithm using adjusted for confounding variables
...
#' @export
#' @examples acsimplsomni <- function(X,Y,A,lambda,npc,resp.kernel="linear", conf.kernel="linear", bandwidth=NULL, c1=NULL, c2=NULL, maxiter=50, delta=10^-8, filter=T, minmaxsep=1e-3)

acsimplsomni <- function(X,Y,A,lambda,npc,resp.kernel=c("linear","delta"), 
                         conf.kernel=c("linear", "gaussian"), bandwidth=NULL, 
                         c1=NULL, c2=NULL, maxiter=50, delta=10^-8, filter=T, 
                         minmaxsep=1e-3){
  ...
  if(is.null(c2)){
    simpls.obj <- acsimpls(X=X,Y=Y,A=A,lambda=lambda,npc=npc,resp.kernel=resp.kernel, conf.kernel=conf.kernel, bandwidth=bandwidth)
    return(simpls.obj)
  } else{
    ssimpls.obj <- acssimpls(X=X,Y=Y,A=A,lambda=lambda,npc=npc,resp.kernel=resp.kernel, conf.kernel=conf.kernel, bandwidth=bandwidth, 
